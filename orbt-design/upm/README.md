...
The **User Position Manager (UPM)** functions as the **ledger and controller** for all user-level interactions within ORBT’s core system. Whenever a user or facilitator interacts with the protocol — such as depositing collateral, minting **0xAssets**, or initiating a **Pocket** strategy — the UPM records these activities as **positions**.
...
Each position maintains a complete state representation, including:

* Assets held as collateral
* Liabilities (e.g., 0xUSD debt or other obligations)
* Linkages to active **Pockets** or strategies

The UPM abstracts complexity for end users by serving as a unified control layer. _For instance_, when a user deposits **ETH** and mints **0xUSD**, the UPM creates a position that links the ETH (tracked by the **Unified Collateral Engine**) with the corresponding 0xUSD debt. Through this mechanism, users can manage or close positions seamlessly through a single interface, without needing to directly interact with multiple contracts or modules.
