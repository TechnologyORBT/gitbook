...
* **Haircuts and Pauses**: Quotes incorporate both `P(asset)` and `H(asset)`. During a depeg, governance can reduce `H(asset)` (or _pause the asset)_, preventing value leakage from 0xUSD holders into a depegged underlying.
