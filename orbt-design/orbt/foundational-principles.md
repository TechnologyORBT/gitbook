...
Instead of siloed pools per chain or per strategy, ORBT’s architecture routes capital dynamically through **Pocket contracts** and the **ORBT Money Market (ORBTMM)**, allowing on-demand access to liquidity wherever it’s needed.
...
ORBT’s interface and protocol logic are built around **intents**, abstracting away multi-step operations such as collateral management, bridging, and execution routing.

Behind each user action, ORBT automatically composes optimal transaction paths, leveraging on-chain liquidity and facilitator strategies.\
...
* The **edge layer**, involving facilitators, institutional pockets, or RWAs, operates under governance approval and controlled access.
...
* All facilitator interactions and Pocket executions are logged immutably.
...
By adhering to ERC standards, supporting cross-chain interoperability (via CCIP, LayerZero, etc.), and enabling data transparency through subgraphs and APIs, ORBT ensures that **its assets (0xUSD, 0xAssets)** and **its architecture** can plug into any ecosystem.
