...
*   **Debt index (global scalar):** `debtIndex` (init `1e18`) converts each allocator’s stored **`baseDebt`** into **effective debt**:
    ...
    $$\displaystyle \text{effectiveDebt}[a] = \text{baseDebt}[a] \times \frac{\text{debtIndex}}{10^{18}}$$
...
* **Crediting vs. repaying:**
  * **Credit mint** adds   $$\displaystyle \text{baseDelta} = \text{amount} \times \frac{10^{18}}{\text{debtIndex}}$$   to the allocator’s `baseDebt` and increments `baseTotalDebt`; their **`reservedZeroX`** increases by `amount`.
  * **Repay (in underlying)** computes an **0x-equivalent principal** by decimals normalization (no oracle), caps to current **effective** debt, then reduces `baseDebt` by $$\displaystyle \text{baseRepay} = \text{repay0x} \times \frac{10^{18}}{\text{debtIndex}}$$ (and decrements `baseTotalDebt`).
...
* **Linked-list ordering for pro-rata operations:**\
  ...
  * During **protocol A→0x** settlement, if unreserved 0x is insufficient, UCE computes pro-rata draws across allocators’ **available capacity** (`min(reservedZeroX, effectiveDebt)`) by **single-pass** traversals of this list, then applies reductions in base units.
