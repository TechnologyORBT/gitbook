...
Pockets are self-contained smart contracts that execute user or facilitator strategies.\
...
Users sign once to express intent; the system executes multiple steps behind the scenes — minting, swapping, staking, etc.
...
* **Active Liquidity (≈75%)** — deployed across Pockets and strategies to generate yield.
* **Reserve Liquidity (≈25%)** — held in UCE and Safety Vaults for instant redemptions and peg defense.
...
Because ORBT spans multiple chains, it synchronizes key states (like 0xUSD supply or facilitator limits) via secure cross-chain messaging (CCIP, LayerZero).\
