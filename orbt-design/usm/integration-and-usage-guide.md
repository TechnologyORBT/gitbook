---
description: >-
  USM (User Staking Module) offers two plug‑and‑play staking primitives that
  integrators and farmers can compose with: sOxAsset and ORBT StakingRewards.
...
├─ deposits 0xAsset ──► sOxAsset (ERC4626 vault)
...
#### A) Integrate **sOxAsset** (ERC‑4626 vault)

1. **Approve & deposit**
   * User approves `underlyingAsset` to the sOxAsset contract
...
* **Interest minting**
  ...
  * **Assumption**: `underlyingAsset` must expose `mint(address,uint256)` and grant minter rights to sOxAsset.
* **Rewards index**
...
```solidity
APR ≈ (rateRay / 1e27)^(secondsPerYear) - 1
```

**APR estimator (rewards):**

```solidity
rewardsPerYear = rewardRatePerSecond * secondsPerYearAPR_rewards ≈ (rewardsPerYear * rewardTokenPrice) / TVL_in_underlying
```

(Price/TVL come from off‑chain sources.)
...
```solidity
rewardsPerYear = rewardRate * secondsPerYearAPR ≈ (rewardsPerYear * rewardTokenPrice) / totalStakedValue
```
...
```solidity
// sOxAsset
uint256 shares = sOxAsset.balanceOf(user);
uint256 assets = sOxAsset.convertToAssets(shares);
uint256 claimable = sOxAsset.claimable(user);
// ORBT StakingRewards
...
```typescript
// sOxAsset deposit
await underlying.approve(sOxAsset.address, amount);
await sOxAsset.deposit(amount, user);
// ORBT stake
...
```typescript
// sOxAsset redeem exact shares
await sOxAsset.redeem(shares, user, user);
// ORBT withdraw
...
```typescript
// sOxAsset
await sOxAsset.claim(user, 0); // 0 = claim all
// ORBT StakingRewards
...
```typescript
const sharesOut = await sOxAsset.previewDeposit(assets);
const assetsOut = await sOxAsset.previewRedeem(shares);
const rateRay = await sOxAsset.previewExchangeRateRay();
```
...
> The reward vault must pre‑fund and `approve` the sOxAsset to `transferFrom` rewards when users claim.
...
* **First depositor**: sOxAsset mints `MIN_LIQUIDITY` shares to `address(1)` to avoid rounding issues. The first real depositor gets 1:1 assets→shares at init.
...
* **Minting requirement**: Underlying must implement `mint(address,uint256)` and grant minter rights to sOxAsset.
...
{"name":"deposit","inputs":[{"name":"assets","type":"uint256"},{"name":"receiver","type":"address"}],"outputs":[{"type":"uint256"}],"stateMutability":"nonpayable","type":"function"},
{"name":"redeem","inputs":[{"name":"shares","type":"uint256"},{"name":"receiver","type":"address"},{"name":"owner","type":"address"}],"outputs":[{"type":"uint256"}],"stateMutability":"nonpayable","type":"function"},
{"name":"convertToAssets","inputs":[{"name":"shares","type":"uint256"}],"outputs":[{"type":"uint256"}],"stateMutability":"view","type":"function"},
{"name":"previewExchangeRateRay","outputs":[{"type":"uint256"}],"stateMutability":"view","type":"function"},
{"name":"claim","inputs":[{"name":"to","type":"address"},{"name":"amount","type":"uint256"}],"outputs":[{"type":"uint256"}],"stateMutability":"nonpayable","type":"function"}
]
...
{"name":"getReward","inputs":[],"outputs":[],"stateMutability":"nonpayable","type":"function"},
{"name":"exit","inputs":[],"outputs":[],"stateMutability":"nonpayable","type":"function"},
{"name":"earned","inputs":[{"name":"account","type":"address"}],"outputs":[{"type":"uint256"}],"stateMutability":"view","type":"function"}
]
```
{% endcode %}
...
* For sOxAsset:
  ...
  * Ensure the **reward vault** is funded and has approved the sOxAsset contract, otherwise `claim` will revert.
...
**Q: Can governance switch to rewards‑only later?**\
