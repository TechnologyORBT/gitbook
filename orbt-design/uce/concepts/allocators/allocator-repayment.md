...
* **Decrease allocator liability**: Convert an on-chain transfer of underlying into an OX-equivalent principal and apply it against the allocator’s **effective debt** (scaled by `debtIndex`).
* **Fund redemptions**: The underlying received remains **on the UCE contract** (on-hand), increasing immediate redemption capacity for OX→U flows.
...
1. **Access / sanity checks**
   ...
   * `assets > 0`, `asset` must be a supported underlying (repay in OX is **disallowed**).
2. **Pull funds**
...
3. **Borrow fee (if configured)**
   * Compute `feeUnderlying = assets * borrowFeeBps / 1e4`.
...
4. **Normalization & cap**
   * Convert principal to **OX-equivalent** via **decimals normalization only** (no oracle):\
     `oxEquivalent = normalizeTo18(asset, principalUnderlying)`.
   * Cap repayment to current **effective debt** (cannot over-repay).
5. **Debt accounting (index-aware)**
   * Convert the OX-equivalent repayment to **base units**:\
     `baseRepay = repayOx * 1e18 / debtIndex`.
...
6. **Event**
   * Emit `AllocatorRepaid(repayer=allocator, allocator=allocator, amount=repayOx)` with the **OX-equivalent principal** applied.

***

### Effects & Invariants

* **On-hand liquidity up**: Underlying stays on the UCE contract, immediately usable for OX→U settlement (step-1 source).
...
* `assets == 0` → revert.
* Unsupported/invalid `asset` or attempting to repay in OX → revert.
