...
* `MAX_REDEMPTION_RATE`: 3–5% ceiling; DECAY\_CONSTANT tuned to \~0.5%/hour effective decay.
...
* **Hard Convertibility**: Oracle-priced, haircut-aware swaps create an always-on primary market. Discounts are corrected by [redemptions ](redemption-rate-and-shortfall-socialization.md)(supply burns with internalized cost to issuers). Premiums are corrected by allocator issuance and user mint-swaps (no redemption-rate friction on issuance).
...
* **Explicit Tail-Loss Sink**: [Debt scaling ](debt-scaling-proportional-socialization-onto-issuers.md)places systemic shortfall where it belongs - on the issuers ([allocators](../allocator/)) in proportion to their open credit - avoiding ambiguous bail-ins and strengthening creditor discipline.
